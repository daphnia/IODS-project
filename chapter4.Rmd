...
```
Now the values are centered around the means and scaled by dividing by standard deviation. 
...
In the output of LDA there can be seen from the last three rows that linear discriminant 1 (LD1) explains 95 % of the between group variance in crime rate categories (per capita crime rate by town).
...
#### 
##Might be needed for drawing (bi)plot: 
...
From the above cross tabulation can be seen that the LDA model created (*lda.fit*) performs quite well only in high and med_high categories as medium_low and low crime rates got about evenly right and wrong predictions.

##To be continued...

## Data Wrangling
...in a R script file create_human.R. To be published at [Github](https://github.com/daphnia/IODS-project/blob/master/data/create_human.R).